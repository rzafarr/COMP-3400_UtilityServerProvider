...
//    sqlite3_open("user_providers.db", &db);
    sqlite3_open("providers.db", &db);

    //create a new provider id
    sqlite3_stmt* countStmt;
    sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM user_providers;", -1, &countStmt, nullptr);
    ...
    std::cout << "Enter Contact Information (Email): ";
...
void ServicesProvider::viewPaidUnpaidBills() {
    ...
    if (sqlite3_prepare_v2(db, paidSQL.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            //string successfulPay = "ssh " + uID + "@delta.cs.uwindsor.ca 'echo -e \"Hello user: " + to_string(userID) + "\\nYour bill id: " + to_string(billID) + " has been paid!\" | mail -s \"Successful Paid Bill\" " + userEmail + "' && ssh " + uID + "@cs3400.cs.uwindsor.ca \"cd ~/Desktop/\"";"";
            //system(successfulPay.c_str());
        }
        sqlite3_finalize(stmt);
    }
    ...
    if (sqlite3_prepare_v2(db, unpaidSQL.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            //string overdueBillMail = "ssh " + uID + "@x.server.ca 'echo -e \"Hello user: " + to_string(userID) + "\\nYour bill id: " + to_string(billID) + " has been paid!\" | mail -s \"Successful Paid Bill\" " + userEmail + "' && ssh " + uID + "@y.server.ca \"cd ~/Desktop/\"";"";
    ...
    sqlite3_close(db);
            ...
            //string overdueBillMail = "ssh -t " + uID + "@x.server.ca 'echo Hello user: " + to_string(userID) + "\nYour bill id: " + to_string(billID) + "is overdue! | mail -s Over Due Bill" + userEmail+ " &&  ssh " + uID +"@y.server.ca \" cd /Desktop/\"'";
            //system (overdueBillMail.c_str());
